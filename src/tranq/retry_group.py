...
class RetryGroup:
    ...
    def __init__(self, *funcs: Callable, on=Exception, retry: int = 0, delay: float = 0.0,
                 ...
                 retry_if=None, on_error=None, circuit_breaker=None,
                 stateful: bool = False, reporters: Optional[List] = None,
    ...
    def run(self, *args, **kwargs):
        ...
        for key, val in self.inject.items():
        ...
        for attempt in range(attempt_start, attempts):
            try:
                ...
                if should_retry and attempt < attempts - 1:
                    log_msg = self.message.format(attempt=attempt + 1) if self.message \
                        else f"Retry group error: {e} (attempt {attempt + 1}/{attempts})"
                    logger.log(self.log_level, log_msg)
                    for rep in self.reporters:
                        rep.report(e, {"func": "retry_group", "attempt": attempt + 1})
                    if cb and not is_async_cb:
                        cb.record_failure()
                    t = compute_backoff(attempt, self.delay, self.backoff,
...
class AsyncRetryGroup:
    """All-or-nothing retry group for async functions (mixed sync/async)."""
